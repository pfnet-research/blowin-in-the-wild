...
def _update_param_with_optimizer(
    ...
    for name in names:
        # breakpoint()
        # skip shot_embeddings
        if name == "shot_embeddings":
            return
...
def duplicate(
    ...
    def param_fn(name: str, p: Tensor) -> Tensor:
        try:
            # _ = p.to(device)[sel.to(device)]
            return torch.nn.Parameter(torch.cat([p, p[sel]]))
        except Exception as e:
            print("shot_embeddings cant be duplicated.", e)
            # p_expanded = p.repeat(len(mask), 1)
            # return torch.nn.Parameter(torch.cat([p_expanded, p_expanded[sel]]))

    def optimizer_fn(key: str, v: Tensor) -> Tensor:
        return torch.cat([v.to(device), torch.zeros((len(sel), *v.shape[1:]), device=device)])
