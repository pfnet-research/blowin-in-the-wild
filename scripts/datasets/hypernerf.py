...
def load_hypernerf_data(data_dir):
    splits = ["train", "val"]
    metas = {}
    
    with open(os.path.join(data_dir,"dataset.json"), "r") as fp:
        file = json.load(fp)
        for s in splits:
            metas[s] = file["{}_ids".format(s)]
    ...
    for s in splits:
        ...
        print("reading hypernerf dataset...")
        for i, frame in enumerate(tqdm.tqdm(meta)):
            ...
            with open(posename, "r") as p:
                ...
                # position = -position @ camtoworld
                transform = make_transform_matrix(camtoworld, posefile["position"])
                # transform = make_transform_matrix(camtoworld, position)
                hwfs.append([h,w,f])
...
class HyperNeRFDataset:
    """HyperNeRF dataset class."""
    ...
    def __init__(
        ...
        if use_colmap:
            ...
            _, _, times, _, i_split, _ = load_hypernerf_data(data_dir)
            self.Ks = Ks
        else:
            images, poses, times, hwfs, i_split, principal_points = load_hypernerf_data(data_dir)
            self.hwfs = hwfs
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", type=str, default="./hypernerf_dataset/vrig-chicken")
    args = parser.parse_args()

    # Parse iphone data.
    dataset = HyperNeRFDataset(data_dir=args.data_dir, split="train")
    ...
    writer = imageio.get_writer("results/hypernerf.mp4", fps=30)
