...
class DefaultStrategy(Strategy):
    ...
    def step_post_backward(
        ...
        if step > self.refine_start_iter and step % self.refine_every == 0:
            # breakpoint()
            # grow GSs
            n_dupli, n_split = self._grow_gs(params, optimizers, state, step) # error point
            if self.verbose:
                print(
    ...
    def _grow_gs(
        ...
        # first duplicate
        if n_dupli > 0:
            duplicate(params=params, optimizers=optimizers, state=state, mask=is_dupli) # error point
